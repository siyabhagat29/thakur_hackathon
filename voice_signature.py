...
def verify_speech(expected_text):
    ...
    if not transcribed_text:
        return False
    print(f"\U0001F50D Recognized: '{transcribed_text}'")
...
def main():
    ...
    for sentence in selected_sentences:
        if not verify_speech(sentence):
            ...
            return
        voice_signatures.append(extract_voice_features(f"{sentence.replace(' ', '_')}.wav"))
    ...
    if not transcribed_signature:
        ...
        return
    print(f"\U0001F50D Detected Signature: '{transcribed_signature}'")
    ...
    if not verification_text:
        ...
        return
    print(f"\U0001F50D Recognized: '{verification_text}'")
